...
# Parameterss: to_log -- the string with the message to log.
#              file_name -- name of the log file to write to
#              verbose_arg -- the command line arg; should the message be  
...
def terminate_main(log_file, verbose):
    log_message("\n\nExiting at: " + str(datetime.datetime.now()), log_file, 
    	verbose)
    log_message("\n------------------------------------------------------------",
    	log_file, verbose)
    exit(0) 
...
# Paramters: abs_dir -- the absolute dir of the file to be copied,
#            new_dir -- the new directory for the copied file, 
#            new_name -- the name for the new file.
...
# Given the zipped list of dates, camera ids, and absolute paths, remove all the
# entries with the X most recent dates where X is an argument.
# Parameters: date_cams_abspaths -- zipped list of dates, cam ids, and absolute
#				    paths.  
#             most_recent_num -- how many of the most recent dates to remove
#				 (only the most recent date? the 2 most recent?)
# Return: date_cams_abspaths with the 'most_recent_num' most recent entries
# removed
def remove_most_recent(date_cams_abspaths, most_recent_num, log_file, verbose):
    log_message("\nRemoving the most recent directories from processing'
    	' (they're possibly still recording):", log_file, verbose)
...
# Updates the table processing current data in directory
# Parameters: currently_processing --
#             tiger_count --
def update_table(currently_processing, tiger_count):
...
# Processes all the directories that need to be processed and updates tiger 
# database
# Parameters: abs_path --
#             vid_path --
#             new_dir --
#             file_name --
def process_dir(abs_path, vid_path, new_dir, file_name):
    for item in need_processing:
	tiger_count = 0   
    	currently_processing = item[0]
    	log_message("\nProcessing this directory: " + currently_processing, 
		args.log_file, args.verbose)
                                                # dir, min_size, hits
    	counts_and_dirs = getPositives.retClips(currently_processing, 500, 10)
    	...
    	for elements in counts_and_dirs:
            ...
            abs_path = elements[1]
            date_path, file_name = video_retriever.date_and_file_name(abs_path,
	    	args.video_path)
            new_dir = args.saved_activity + "/" + date_path
            copy_file(abs_path, new_dir, file_name)

            # now we know where pos footage was saved at
            tiger_log.update_savedat_by_dir(currently_processing, new_dir)      

       # update the table for this directory  
       update_table(currently_processing, tiger_count)

 
def main ():
    # build the command-line parser, parse
    parser = argparse.ArgumentParser(description='Process airVision security'
	' footage for activity.')
    parser.add_argument('video_path', help='Path to airVision Videos'
	' directory.')
    parser.add_argument('saved_activity', help='Path to directory to save'
        ' active videos under.')
    parser.add_argument('log_file', help='File to put logging messages in.')
    parser.add_argument('--verbose', action='store_true', help='If verbose is'
        ' given, logging messages will be printed to stdout.')
    parser.add_argument('--force', action='store_true', help="The logger'
        ' usually does not process the most recently recorded video'
	' directories so as to avoid 'finishing' processsing on a'
	' directory that still may be recording footage.  --force'
	' forces the logger to process ALL discovered directories.'
	' [--force is useful if recording has stopped indefinitely.]")
    args = parser.parse_args()  

    # begin processing footage
    log_message("\n\n\n----------------------------------------------------'
    	'--------", args.log_file, args.verbose)
    log_message("\nBeginning: " + str(datetime.datetime.now()), args.log_file,
    	args.verbose) 
    log_message("\nProcessing airVision data at: " + args.video_path, 
    	args.log_file, args.verbose)
    log_message("\nSaving positive footage at: " + args.saved_activity, 
    	args.log_file, args.verbose)
   
    # grab the dates of the videos (2013/10/22/13), camera_ids, and absolut paths
    dates, camera_ids, abs_paths = video_retriever.grab_video_dirs(
    	args.video_path)

    # retrieve, zip together footage info
    if len(dates) != len(camera_ids):
        log_message("\nThe number of returned dates from the airVision directory" 
        	" does not equal the number of camera_ids.  Exiting...", 
                args.log_file, args.verbose)
    ...
    if args.force == False:         
        date_cams_abspaths = remove_most_recent(date_cams_abspaths, 3, 
		args.log_file, args.verbose)
    else:
        log_message("\nForced argument given. Processing all directories " 
        	"regardless of how recent they are.", args.log_file, args.verbose)
    ...
    log_message("\nSelecting unprocessed rows from tiger_log.db table...", 
    	args.log_file, args.verbose)
    ...
    log_message("\n", args.log_file, args.verbose)
    process_dir(abs_path, args.vide_path, new_dir, file_name)
